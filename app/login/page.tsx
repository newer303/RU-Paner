        ...
        <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between px-1">
            <div className="flex items-center gap-2">
              {/* Google Sign-In Button - removed for now as icon not available */}
              <button
                onClick={() => signIn('google', { callbackUrl: '/' })}
                disabled={loading}
                className="flex w-full items-center justify-center gap-2 rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-950/50 px-4 py-2 text-sm font-semibold text-gray-700 dark:text-zinc-300 hover:bg-gray-50 dark:hover:bg-zinc-900/70 transition-all"
              >
                Sign in with Google
              </button>
            </div>
