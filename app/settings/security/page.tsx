...
export default function ChangePasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (newPassword !== confirmPassword) {
      setStatus('error');
      setMessage('รหัสผ่านใหม่ไม่ตรงกัน กรุณาตรวจสอบอีกครั้ง');
      return;
    }
      ...
      <div className="relative w-full max-w-md space-y-8 rounded-2xl border border-gray-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/80 p-8 shadow-2xl backdrop-blur-sm">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-500 dark:text-zinc-400">
            เพิ่มความปลอดภัยให้บัญชีของคุณด้วยการเปลี่ยนรหัสผ่านใหม่
          </p>
        </div>
        ...
        {status === 'success' ? (
          ...
          <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-semibold text-gray-700 dark:text-zinc-300 ml-1" htmlFor="currentPassword">
                รหัสผ่านปัจจุบัน
              </label>
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-semibold text-gray-700 dark:text-zinc-300 ml-1" htmlFor="newPassword">
                รหัสผ่านใหม่
              </label>
              <div className="relative group">
                <Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400 group-focus-within:text-blue-500 transition-colors" />
