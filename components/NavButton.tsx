...
export const NavButton = ({ active, onClick, icon, label, className = "" }: NavButtonProps) => (
  <button
    onClick={onClick}
    className={`flex flex-col md:flex-row items-center gap-2 md:gap-4 p-3 md:p-4 rounded-2xl md:rounded-[1.25rem] transition-all duration-300 relative overflow-hidden group ${active
  ...
  >
    ...
    <span className="text-[9px] md:text-xs font-black uppercase tracking-wider relative z-10">{label}</span>
    {active && (
